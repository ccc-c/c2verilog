...
int aluExt(int16_t c, int16_t A, int16_t D, int16_t AM) {
  ...
  switch (c) {
    ...
    // 0x30-33 已被使用，以下從 0x34-0x36, 0x38-0x39, 0x3B-0x3E 可用
    case 0x34: out=0; PC = ILR; break; // {"iret", "110100"},
...
void cpu() {
  ...
  debug("\n");
  iCount ++; // 已執行指令數
  uint32_t tiPeriod = ((uint32_t) CR1) << 10; // 快速的乘以 128
  if (!inInterrupt && tiPeriod > 0 && (iCount % tiPeriod == 0)) {
